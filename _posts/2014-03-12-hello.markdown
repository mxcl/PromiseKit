...
Errors and exceptions break the chain of promises and bubble up to the next `catch` handler. Any thrown objects are caught and wrapped in an NSError object. The original thrown object is accessible via `error.userInfo[PMKUnderlyingExceptionKey]`.
...
* Returning an `NSError *` from a `then` or `catch` rejects the returned `Promise`

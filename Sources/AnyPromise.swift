...
@objc(AnyPromise) public class AnyPromise: NSObject {
    ...
    */
    public func then<T>(on q: dispatch_queue_t = create_default_promise_dispatch_queue(), body: (AnyObject?) throws -> T) -> Promise<T> {
    ...
    /**
     Continue a Promise<T> chain from an AnyPromise.
    */
    public func then(on q: dispatch_queue_t = create_default_promise_dispatch_queue(), body: (AnyObject?) -> AnyPromise) -> Promise<AnyObject?> {
    ...
    /**
     Continue a Promise<T> chain from an AnyPromise.
    */
    public func then<T>(on q: dispatch_queue_t = create_default_promise_dispatch_queue(), body: (AnyObject?) -> Promise<T>) -> Promise<T> {
        return Promise(sealant: { resolve in
            pipe { object in
                if let error = object as? NSError {
                    resolve(.Rejected(error, error.token))

...
extension XCTestCase {
    func suiteFulfilled(numberOfExpectations:Int, test:(Promise<Int>, [XCTestExpectation!], Int)->Void) {
        ...
        let (p2, f2, _) = Promise<Int>.pendingPromise()
        let v2 = Int(rand())
        ...
        let (p3, f3, _) = Promise<Int>.pendingPromise()
        let v3 = Int(rand())
    ...
    func suiteRejected(numberOfExpectations:Int, test:(Promise<Int>, [XCTestExpectation!], NSError)->Void) {
        ...
        let (p2, _, r2) = Promise<Int>.pendingPromise()
        ...
        let (p3, _, r3) = Promise<Int>.pendingPromise()
